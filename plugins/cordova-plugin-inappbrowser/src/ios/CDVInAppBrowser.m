...
- (void)createViews
{
    ...
    self.spinner.backgroundColor = [UIColor colorWithWhite:1.0f alpha:1.0f];
    self.spinner.clearsContextBeforeDrawing = NO;
    self.spinner.clipsToBounds = NO;
    self.spinner.contentMode = UIViewContentModeScaleToFill;
    self.spinner.frame = CGRectMake(0, 0, self.view.frame.size.width, self.view.frame.size.height);
...
- (void)webViewDidFinishLoad:(UIWebView*)theWebView
{
    ...
    // TS: If the workgroup cookie is present, sign in is completed, close the browser.
    NSHTTPCookie *cookie;
    NSHTTPCookieStorage *storage = [NSHTTPCookieStorage sharedHTTPCookieStorage];
    BOOL sessionCookie = NO;
    for (cookie in [storage cookies])
    {
        if ([cookie.name isEqualToString:@"workgroup_session_id"] && [cookie.value length] > 10) {
            sessionCookie = YES;
            break;
        }
    }
